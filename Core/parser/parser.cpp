...
namespace skiff
{
    ...
    void track_brace(token_type tkn, stack<token_type> *braces)
    {
        switch (tkn)
        {
            ...
            case token_type::LEFT_BRACKET:braces->push(tkn);
                break;
            case token_type::RIGHT_BRACE:check_back_brace(token_type::LEFT_BRACE, braces);
                break;
            case token_type::RIGHT_PAREN:check_back_brace(token_type::LEFT_PAREN, braces);
                break;
            case token_type::RIGHT_BRACKET:check_back_brace(token_type::LEFT_BRACKET, braces);
                break;
            default:break;
        }
    }
    ...
    void track_generic_brace(token_type tkn, stack<token_type> *braces)
    {
        switch (tkn)
        {
            case token_type::LEFT_ANGLE_BRACE:braces->push(tkn);
                break;
            case token_type::RIGHT_ANGLE_BRACE:check_back_brace(token_type::LEFT_ANGLE_BRACE, braces);
                break;
            default:break;
        }
    }
    ...
    parse_match *parse_pattern::match_with_brace_track(size_t strt, vector<token> tokens,
                                                       std::function<void(token_type,
                                                                          stack<token_type> *)> brace_tracker)
    {
        size_t rule_pos = 0;
    ...
    vector<statements::function_definition::function_parameter> parse_function_params(vector<token> tokens)
    {
        parse_pattern FX_PARAM =
                parse_pattern(token_type::NAME).then(token_type::COLON).then(token_type::NAME).terminate(
                        token_type::COMMA);
...
//                );
    ...
    vector<statement *> parser::parse()
    {
        ...
        parse_pattern_logic TYPE_DEF = parse_pattern_logic(token_type::NAME).maybe(token_type::LEFT_ANGLE_BRACE)
                .maybe(token_type::RIGHT_ANGLE_BRACE).maybe(token_type::COMMA).maybe(token_type::DOT).maybe(
                        token_type::COLON);
        ...
        parse_pattern DECLARE =
                parse_pattern(token_type::NAME).then(token_type::COLON).capture(TYPE_CALL).terminate(
        ...
        parse_pattern FUNCTION_CALL =
                parse_pattern(TYPE_CALL, parse_pattern_type::CAPTURE).then(token_type::LEFT_PAREN).capture().then(
        ...
        parse_pattern EXTERN_FUNCTION_DEF =
                parse_pattern(token_type::EXTERN).then(token_type::DEF).then(token_type::NAME).capture().then(
                                token_type::LEFT_PAREN).capture()
                        .then(token_type::RIGHT_PAREN).then(token_type::COLON).then(token_type::NAME).then(
                                token_type::SEMICOLON);

        parse_pattern CLASS_DEF =
                parse_pattern(parse_pattern_logic(token_type::CLASS).maybe(token_type::STRUCT),
        ...
        parse_pattern NEW_QUCK_INIT =
                parse_pattern(token_type::NEW).then(token_type::NAME).capture().then(token_type::LEFT_PAREN)
                        .capture().then(token_type::RIGHT_PAREN).then(token_type::LEFT_BRACE).then(
                                token_type::LEFT_BRACE).capture()
        ...
        parse_pattern BIT_SHIFT_RIGHT =
                parse_pattern().then(token_type::RIGHT_ANGLE_BRACE).then(
                        token_type::RIGHT_ANGLE_BRACE).capture().terminate(token_type::SEMICOLON);
        ...
        parse_pattern PRE_DEC =
                parse_pattern(parse_pattern_logic(token_type::INC).maybe(token_type::DEC),
                              parse_pattern_type::MULTIMATCH)
                        .capture().terminate(token_type::SEMICOLON);
        ...
        parse_pattern ELSE =
                parse_pattern(token_type::ELSE).capture().then(token_type::LEFT_BRACE).capture().then(
        ...
        parse_pattern BOOLEAN_LITERAL =
                parse_pattern(parse_pattern_logic(token_type::TRU).maybe(token_type::FALS),
                              parse_pattern_type::MULTIMATCH)
                        .terminate(token_type::SEMICOLON);
...
//                        ));
            ...
            if (cap)
            {
                ...
                switch (cap->selected_tokens.at(0).get_type())
                {
                    case token_type::BREAK:op = type::BREAK;
                        break;
                    case token_type::NEXT:op = type::NEXT;
                        break;
            ...
            if (cap)
            {
                ...
                switch (cap->selected_tokens.at(0).get_type())
                {
                    case token_type::DOUBLE_EQUAL:cp = cap_type::EQUAL;
                        break;
                    case token_type::BANG_EQUAL:cp = cap_type::NOT_EQUAL;
                        break;
                    case token_type::LEFT_ANGLE_BRACE:cp = cap_type::LESS_THAN;
                        break;
                    case token_type::LESS_THAN_EQUAL:cp = cap_type::LESS_THAN_EQUAL_TO;
                        break;
                    case token_type::RIGHT_ANGLE_BRACE:cp = cap_type::GREATER_THAN;
                        break;
                    case token_type::GREATER_THAN_EQUAL:cp = cap_type::GREATER_THAN_EQUAL_TO;
                        break;
            ...
            if (cap)
            {
                ...
                switch (cap->selected_tokens.at(0).get_type())
                {
                    case token_type::BIT_AND:op = bit_op::AND;
                        break;
                    case token_type::BIT_OR:op = bit_op::OR;
                        break;
                    case token_type::BIT_XOR:op = bit_op::XOR;
                        break;
...
//                                    ));
                    ...
                    default:std::cout << "Unknown loop in flow match" << std::endl;
                        break;
                }
            ...
            if (cap)
            {
                ...
                switch (cap->selected_tokens.at(0).get_type())
                {
                    case token_type::STAR:opr = statements::math_statement::MUL;
                        break;
                    case token_type::DIV:opr = statements::math_statement::DIV;
                        break;
                    default:std::cout << "Bad math op for div mul" << std::endl;
                        break;
                }
                statements.push_back(
                        new statements::math_statement(
            ...
            if (cap)
            {
                ...
                switch (cap->selected_tokens.at(0).get_type())
                {
                    case token_type::PLUS:op = statements::math_statement::ADD;
                        break;
                    case token_type::MINUS:op = statements::math_statement::SUB;
                        break;
                    default:std::cout << "Bad math op for add sub" << std::endl;
                        break;
                }
                statements.push_back(
                        new statements::math_statement(
            ...
            if (cap)
            {
                ...
                switch (cap->selected_tokens.at(0).get_type())
                {
                    case token_type::INC:op = statements::self_modifier::modifier_type::PLUS;
                        break;
                    case token_type::DEC:op = statements::self_modifier::modifier_type::MINUS;
                        break;
                    default:std::cout << "Bad symbol for self mod" << std::endl;
                        break;
                }
